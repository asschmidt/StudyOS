...
.set PIC2_CMD_REG, PIC2_IO_ADDR         /* IO Port Address for PIC2 Command Register */
.set PIC2_DATA_REG, PIC2_IO_ADDR + 1    /* IO Port Address for PIC2 Data Register */


/* PIC Commands */
.set PIC_EOI,           0x20            /* End of Interrupt Command */
.set PIC_READ_IRR,      0x0a            /* OCW3 irq ready next CMD read */
.set PIC_READ_ISR,      0x0b            /* OCW3 irq service next CMD read */

/* PIC onfigure Bit Values */
.set ICW1_ICW4,	        0x01		    /* Indicates that ICW4 will be present */
.set ICW1_SINGLE,	    0x02		    /* Single (cascade) mode */
.set ICW1_INTERVAL4,	0x04		    /* Call address interval 4 (8) */
.set ICW1_LEVEL,	    0x08		    /* Level triggered (edge) mode */
.set ICW1_INIT,	        0x10		    /* Initialization - required! */

.set ICW4_8086,	        0x01		    /* 8086/88 (MCS-80/85) mode */
.set ICW4_AUTO,	        0x02		    /* Auto (normal) EOI */
.set ICW4_BUF_SLAVE,	0x08		    /* Buffered mode/slave */
.set ICW4_BUF_MASTER,	0x0C		    /* Buffered mode/master */
.set ICW4_SFNM,	        0x10		    /* Special fully nested (not) */
